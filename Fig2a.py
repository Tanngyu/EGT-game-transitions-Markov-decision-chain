...
for sigma in [0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.899999]:
    delta = 0.1
    mu = 1 - sigma
    ...
    x1.append(np.mean(last_elements_PDG))
    x2.append(np.mean(last_elements_SDG))
    x3.append(np.mean(last_elements_SHG))


plt.rcParams['font.family'] = 'Times New Roman'
plt.rcParams['font.size'] = 48
markers = ['o', 's', '^', 'D','p' ]
x_values = [0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9]
plt.figure(figsize=(12, 6))
plt.plot(x_values,x1, label=r'$G_1$', marker=markers[0],color='#5F9C61',linewidth=4,markersize=10)
plt.plot(x_values,x2, label=r'$G_2$', marker=markers[1],color='#B092B6',linewidth=4,markersize=10)
plt.plot(x_values,x3, label=r'$G_3$', marker=markers[2],color='#E38D26',linewidth=4,markersize=10)

plt.fill_between(x_values,x3, color='#E38D26', alpha=0.3)
plt.fill_between(x_values,x2,  color='#B092B6', alpha=0.3)
plt.fill_between(x_values,x1,  color='#5F9C61', alpha=0.3)

plt.title('')
plt.xlabel(r'$\sigma$')
plt.ylabel(r'$\rho({G_i})$')
plt.legend(loc='upper center',prop={'size': 32})
